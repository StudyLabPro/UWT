...
2. **Визуализаторы** — интерактивные объясняющие модели.
3. **Python modeling** — вычислимое ядро проверки, прогнозирования и моделирования.
4. **Browser Python Runner** — исполнимые Python-демо в браузере.
5. **ACT / Balansis** — слой компенсационной арифметики и численной политики.
6. **MagicBrain** — исследовательский и продуктовый модуль, связывающий UWT с нейросетевыми, когнитивными и агентными структурами.
7. **Дизайн-система** — визуальный язык научного атласа, а не декоративная оболочка.
...
Любое изменение должно усиливать эту цепочку.
...
UnifiedWholeTheory/
  ...
  web-app/       -> React/Vite web-платформа, визуализаторы, runner, страницы сайта
  check-all.ps1  -> единая проверка web build, web lint, Python pytest
  SKILL.md       -> этот operational skill для агентов
```
...
## 2. Миссия агентной команды

Сделать UWT-платформу:

- **понятной** — пользователь без подготовки должен войти через простые визуальные объяснения;
...
- **красивой** — интерфейс должен ощущаться как научный атлас живой теории;
- **надёжной** — сборка, lint, typecheck, pytest и ручной QA обязательны для релевантных изменений;
- **расширяемой** — каждая новая страница, визуализация, модель и демо должны встраиваться в общую систему;
- **честной** — пользователь должен видеть, где теория, где гипотеза, где demo, где verified model.
...
- удалять философский и монографический слой ради упрощения;
- заменять термины случайными популярными аналогами;
...
- делать визуально эффектные, но смыслово пустые блоки.

Разрешено:

- адаптировать сложные идеи под разные уровни пользователя;
- добавлять схемы, глоссарий, пояснения и визуализаторы;
- разделять текст на уровни: простой, технический, исследовательский;
- предлагать уточнение формулировок, если это повышает строгость;
- маркировать спорные тезисы как гипотезы.
...
| `VERIFIED` | поведение, покрытое тестом или воспроизводимым экспериментом |
| `EXPERIMENT` | сценарий запуска, создающий данные/JSON/график |
| `INTERPRETATION` | объяснение результата, не равное доказательству |

В UI желательно показывать бейджи: `Теория`, `Гипотеза`, `Демо`, `Модель`, `Проверено`, `Эксперимент`, `Интерпретация`.

### 3.3. Проверки должны быть неинтерактивными

Проверочная последовательность не должна содержать долгоживущие процессы. Особенно запрещено включать `npm run dev` в блок воспроизводимых проверок, потому что в `web-app` он запускает Vite dev server и блокирует дальнейшее выполнение.

Правильная полная проверка проекта:
...
Локальный интерактивный preview/dev server запускать отдельно:
...
Комментарий: `npm run dev` — интерактивная команда для ручной разработки и просмотра сайта. Она не является частью автоматического check sequence.
...
- связь монографии, визуализаторов, моделей и исполнимых демо;
- приоритеты задач;
- разделение `demo`, `hypothesis`, `model`, `verified`;
- постановку issues для команды;
- принятие решений, когда нужен новый раздел, а когда достаточно блока внутри существующего.
...
- есть ли связь с монографией или исследовательской задачей.
...
- корректность связей между `theory/`, `papers/`, `modeling/` и `web-app/`.
...
- не добавлять зависимость, если её можно избежать;
- не оставлять unused imports и dead code.
...
- делать вид, что browser demo равно полноценному backend execution.
...
- glassmorphism допустим, но не должен снижать читаемость;
...
Минимальная проверка перед завершением документационно-независимой задачи:
...
Все анимации должны уважать reduced motion:
...
`modeling/` — вычислимое ядро проекта. Здесь UWT становится проверяемой через:
...
Не использовать цвет как единственный способ передачи смысла. Добавлять текст, иконки или паттерны.
...
Важно: `npm run dev` не включать в PR checklist как check. Это интерактивный local preview.
...
- помещать `npm run dev` в автоматическую check sequence;
...
Важно: не писать `npm run dev` как passed check. Можно написать отдельно: `local preview via npm run dev — manually used / not used`.
