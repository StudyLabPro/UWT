...
* **No em-dashes inside the quote text** as design flourish (long pauses, kinetic em-dashes, em-dash-bullets). See `references/ai-tells.md` Section 9.G - em-dash is completely banned.
...
The full catalogue of forbidden AI-tell patterns (visual/CSS, typography, layout, content, external resources, production-test tells, and the non-negotiable em-dash ban in 9.G) is vendored reference content, kept out of this file to save context: see `references/ai-tells.md`. Load it before any Pre-Flight Check (Section 14) or when auditing a redesign against AI tells.

---

## 10. Reference Vocabulary

Pattern names the agent should know (hero paradigms, navigation/menus, layout/grids, cards, scroll animations, galleries/media, typography, micro-interactions, and animation-library choice) are vendored reference content: see `references/pattern-vocabulary.md`. Load it when the design read calls for naming or reaching for a specific pattern, or when populating the Block Library (Section 12).
...
5. **Hero & key-section recomposition** - restructure top-of-funnel using the pattern vocabulary in `references/pattern-vocabulary.md`.
...
The Reference Vocabulary (`references/pattern-vocabulary.md`, see Section 10) names patterns. The Block Library implements them with real props, real motion specs, and real code sketches.
...
- [ ] **ZERO em-dashes (`—`) anywhere on the page.** Headlines, eyebrows, pills, body, quotes, attribution, captions, buttons, alt text. Zero. (`references/ai-tells.md` Section 9.G - non-negotiable.)
...
- [ ] **No AI Tells** from `references/ai-tells.md` (Inter as default, AI-purple, three-equal cards, Jane Doe, Acme, "Quietly in use at")?
...
## Appendices - Vendored Reference Material

Real install commands, canonical doc links per design system (Section 2), and an honest Apple Liquid Glass web approximation are vendored reference content, kept out of this file to save context: see `references/vendored-examples.md`.
