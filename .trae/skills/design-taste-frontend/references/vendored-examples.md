# Vendored Reference Material — Design System Appendices

The sections below are vendored reference content, called from `design-taste-frontend/SKILL.md` Section 2 (Brief → Design System Map). They give the agent real install commands, real canonical doc links, and real working starter snippets for each design system named there. Use them to ground decisions in production reality, not training-data fiction.
...
**End of appendices.** Install commands above are reality anchors. The Apple Liquid Glass skeleton is a labeled approximation, not an Apple-issued package. For canonical docs per design system, consult the system's official docs (links in `design-taste-frontend/SKILL.md` Section 2, plus Appendix B above).
