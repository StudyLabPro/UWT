...
- `modeling`: `python -m pytest` — успешно, `2 passed`.
...
- Python-тесты расширены с 2 до 19: добавлено покрытие forecast, parameter scan, serialization, Balansis adapter, CLI и граничных значений `UWTConfig`.
- Добавлена строгая валидация `UWTConfig`.
- Устранён pytest-asyncio warning через `asyncio_default_fixture_loop_scope = "function"`.
